...
# Using savefig parameters instead of global figure.dpi to keep web UI fast while exports are high-res
...
# --- CACHING & DOWNLOAD HELPER FUNCTIONS ---

@st.cache_data(show_spinner=False)
def generate_table_image_bytes(df_target):
    """Generates a high-res table image and caches it in memory."""
...
def render_table_as_img_download(df_target, file_prefix, key):
    """Renders the download button using the cached high-res image."""
    if df_target is None or df_target.empty:
        return
    
    img_bytes = generate_table_image_bytes(df_target)
    
    st.download_button(
        label=f"🖼️ Download {file_prefix} Table (High-Res PNG)",
        data=img_bytes,
...
def render_download_img(fig_target, file_prefix, key):
    """Renders the download button for Matplotlib charts."""
    img_buffer = io.BytesIO()
    fig_target.savefig(img_buffer, format="png", dpi=300, bbox_inches='tight')
    st.download_button(
        label=f"📥 Download {file_prefix} Chart (High-Res PNG)",
...
if uploaded_file is not None:
    ...
    df = load_and_preprocess_data(file_bytes)
    ...
    with tab6:
        ...
        if USAGE_COL and COIL_ID_COL in df.columns and LEN_COL in df.columns and SCRAP_COL in df.columns: 
            ...
            df_t6['Usage_Date'] = safe_parse_usage_date(df_t6[USAGE_COL])
            ...
            if df_t6.empty:
                ...
                for prod in prod_periods:
                    ...
                    for usage in usage_months:
                        cell_data = matrix_data[(matrix_data['Usage_Month'] == usage) & (matrix_data['Time_Group'] == prod)]
                        if cell_data.empty:
                            html_matrix += "<td style='background-color: #fafafa; color: #aaa; text-align:center; vertical-align:middle;'>No Data</td>"
                        else:
                            ...
                            grade_html = ""
                            total_coils = row['Total_Coils']
                            if total_coils > 0:
                                for g in available_grades:
                                    g_pct = (row[g] / total_coils * 100)
                                    if g_pct > 0:
            ...
            if split_coils:
                ...
                for coil in split_coils:
                    ...
                    old_val = df_c[df_c['Machine_Status'] == 'Old Machine (< Apr 2026)']['Scrap_Rate'].values[0]
                    new_val = df_c[df_c['Machine_Status'] == 'New Machine (>= Apr 2026)']['Scrap_Rate'].values[0]
                    ...
                    if old_val > 10 and new_val < 5: 
                        ...
                        root = "⚠️ Material / Process Issue"
                    elif new_val > old_val + 5: 
                        root = "⚙️ New Machine Tuning Issue"
                    elif old_val > 0 and new_val == 0: 
                        root = "✅ Improved on New Machine"
                    else: 
                        root = "✅ Normal / Stable"
                        
                    split_data.append({
                        'Coil ID': coil, 
                        'Scrap (Old Machine)': old_val, 
                        'Scrap (New Machine)': new_val,
