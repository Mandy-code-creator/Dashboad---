...
# ==========================================================
# HELPER: Tải ảnh PNG chất lượng cao từ matplotlib figure
# ==========================================================
def fig_to_png_bytes(fig: plt.Figure, dpi: int = 300) -> bytes:
    """Xuất matplotlib figure thành PNG bytes độ phân giải cao."""
    buf = io.BytesIO()
    fig.savefig(buf, format='png', dpi=dpi, bbox_inches='tight', facecolor='white')
    buf.seek(0)
    return buf.read()

def download_chart_btn(fig: plt.Figure, filename: str, label: str = "⬇️ Tải ảnh PNG"):
    """Hiển thị nút download PNG ngay dưới biểu đồ."""
    ...
    st.download_button(
        ...
        file_name=filename,
        mime="image/png",
        key=filename,  # unique key per button
    )

# ==========================================================
# HELPER: Xuất bảng HTML thành PNG cao cấp qua imgkit
# ==========================================================
IMGKIT_OPTIONS = {
    "zoom": "3",
    "width": "1800",
...
def html_table_to_png_bytes(html_content: str, title: str = "") -> bytes:
    """Render bảng HTML thành PNG bytes độ phân giải cao (zoom 3x)."""
    full_html = f"""<!DOCTYPE html><html><head><meta charset='utf-8'>
    <style>
        body {{ font-family: Arial, sans-serif; background: white; padding: 20px; }}
        .export-title {{ font-size: 18px; font-weight: bold; color: #1a3a5c; margin-bottom: 12px; }}
    </style>
    </head><body>
    {"<div class='export-title'>" + title + "</div>" if title else ""}
    {html_content}
    </body></html>"""
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
        tmp_path = tmp.name
    try:
        imgkit.from_string(full_html, tmp_path, options=IMGKIT_OPTIONS)
...
def download_table_btn(html_content: str, filename: str, title: str = "", label: str = "⬇️ Tải bảng PNG"):
    """Hiển thị nút download PNG cho bảng HTML."""
    if st.button(label, key=f"btn_{filename}"):
        with st.spinner("Đang render ảnh chất lượng cao…"):
            png_bytes = html_table_to_png_bytes(html_content, title)
        st.download_button(
            label="📥 Nhấn để tải",
            data=png_bytes,
            file_name=filename,
...
if uploaded_file is not None:
    ...
    for g in base_grades:
        match_cols = [c for c in df.columns if str(c).strip() in [g, f"{g}個數"] or str(c).strip().startswith(f"{g}.")]
    ...
    df = df[(df['Total_Qty'] > 0) | (df.get(LEN_COL, pd.Series(0, index=df.index)) > 0) | (df.get(SCRAP_COL, pd.Series(0, index=df.index)) > 0)]
    ...
    def is_valid_for_control(period_label):
        return not any(x in period_label for x in ["2024", "2025 H1", "2025 Q3", "2025 (Full Year)"])

    def calc_capability(values, feat, period_label, thickness):
        vals = np.array(values, dtype=float)
        vals = vals[~np.isnan(vals)]
        if thickness == 'Overall':
            return {'mean': np.mean(vals) if len(vals) > 0 else 0, 'std': np.std(vals, ddof=1) if len(vals) > 1 else 0,
                    'n': len(vals), 'Cp': None, 'Cpk': None, 'Ca': None, 'LSL': None, 'USL': None, 'Target': None}
        ...
        result = {'mean': mu, 'std': std, 'n': len(vals), 'Cp': None, 'Cpk': None, 'Ca': None, 'LSL': lsl, 'USL': usl, 'Target': tgt}
        if not is_valid_for_control(period_label): return result
        if lsl is not None and usl is not None:
            ...
            result['Ca'] = round((mu - mid) / ((usl - lsl) / 2) * 100, 2)
        elif lsl is not None:
            cpl = (mu - lsl) / (3 * std)
            result['Cp'] = result['Cpk'] = round(cpl, 3)
        elif usl is not None:
            cpu = (usl - mu) / (3 * std)
            result['Cp'] = result['Cpk'] = round(cpu, 3)
        return result
    ...
    def render_capability_badge(cap, feat, period_label, thickness):
        ...
        if thickness == 'Overall':
            cp_v = cpk_v = ca_v = 'N/A'
            clr, lbl, lsl_v, usl_v = '#888888', 'Mixed Thickness (No Global Limit)', '—', '—'
            ...
            cp_v = cpk_v = ca_v = 'N/A'
            clr, lbl, lsl_v, usl_v = '#888888', 'Pre-Q4 2025 (Not Monitored)', '—', '—'
        st.markdown(f"""
        <div style="background:#f8f9fa;border-left:5px solid {clr};border-radius:6px;padding:8px 14px;margin:4px 0 10px 0;font-family:monospace;font-size:13px;line-height:1.8;">
          <span style="font-size:14px;font-weight:bold;color:{clr};">{lbl}</span>&nbsp;&nbsp;|&nbsp;&nbsp;
          <b>LSL</b>: {lsl_v} &nbsp; <b>USL</b>: {usl_v}&nbsp;&nbsp;|&nbsp;&nbsp;
          <b>n</b>: {cap['n']} &nbsp;<b>Mean</b>: {mu_v} &nbsp;<b>Std</b>: {std_v}<br>
          <b style="color:{clr};">Cpk = {cpk_v}</b>&nbsp;&nbsp;<b>Cp = {cp_v}</b>&nbsp;&nbsp;<b>Ca = {ca_v}</b>
    ...
    def build_capability_summary(df_src, feat, label, thick):
        ...
        if not is_valid_for_control(label):
            return {'Period': label, 'Thickness': thick, 'Feature': feat, 'n': cap['n'],
                    'Mean': round(cap['mean'], 2), 'Std': round(cap['std'], 3),
                    'LSL': None, 'USL': None, 'Ca (%)': None, 'Cp': None, 'Cpk': None, 'Verdict': 'Not Monitored'}
        return {'Period': label, 'Thickness': thick, 'Feature': feat, 'n': cap['n'],
                'Mean': round(cap['mean'], 2), 'Std': round(cap['std'], 3),
    ...
    def plot_dist(ax, data, feat, title, y_lim, period_label, thickness):
        ...
        if v_l:
            ...
            for i, info in enumerate(m_info):
                x_pos = positions[i]
                y_pos = y_levels[i]
                ax.annotate(f"{info['v']:.1f}", xy=(info['v'], y_pos * 0.6), xytext=(x_pos, y_pos),
    ...
    with tab1:
        ...
        if 'Production_Date' in df.columns and not df['Production_Date'].isna().all():
            col_m3.metric("Date Range", f"{df['Production_Date'].min().strftime('%Y-%m-%d')} to {df['Production_Date'].max().strftime('%Y-%m-%d')}")
        else:
    ...
    with tab2:
        ...
        yield_summary = df.groupby(['Time_Group', 'Actual_Thickness', 'HR_Material'])[
            ['Total_Qty', 'Acceptable_Qty', 'Severe_Bad_Qty']].sum().reset_index()
        ...
        if not yield_summary.empty:
            ...
            st.dataframe(yield_summary.style
                .background_gradient(subset=['Yield (%)'], cmap='Greens')
                .background_gradient(subset=['Defect_Rate (%)'], cmap='Reds')
                .format({'Actual_Thickness': '{:.2f}', 'Total_Qty': '{:.0f}', 'Acceptable_Qty': '{:.0f}',
                         'Severe_Bad_Qty': '{:.0f}', 'Yield (%)': '{:.2f}%', 'Defect_Rate (%)': '{:.2f}%'}),
        ...
        for period, row in grade_dist_pct_str.iterrows():
            grade_table_html += "<tr><td><b>{}</b></td>".format(period)
            for g in base_grades:
                val = float(row[g].replace('%', ''))
                if g in ['B+', 'B'] and val > 1.0:
                    grade_table_html += f'<td style="color:#c00000;font-weight:bold">{row[g]}</td>'
                elif g in ['A-B+', 'A-B'] and val > 30.0:
                    grade_table_html += f'<td style="color:#2e7d32;font-weight:bold">{row[g]}</td>'
                else:
                    grade_table_html += f'<td>{row[g]}</td>'
            grade_table_html += "</tr>"
        grade_table_html += "</tbody></table>"
        st.markdown(grade_table_html, unsafe_allow_html=True)

        # ── Download nút cho bảng Grade Distribution ──
        download_table_btn(grade_table_html, "Grade_Distribution_Table.png", "Grade Distribution by Time Period (%)")
        ...
        if not grade_dist_display.empty:
            ...
            fig_g.tight_layout()
        st.pyplot(fig_g)
        download_chart_btn(fig_g, "Grade_Distribution_Chart.png")
        ...
        with col_c1:
            ...
            fig_y.tight_layout()
            st.pyplot(fig_y)
            download_chart_btn(fig_y, "Yield_by_Period_Thickness.png")
        ...
        with col_c2:
            ...
            fig_d.tight_layout()
            st.pyplot(fig_d)
            download_chart_btn(fig_d, "Defect_Rate_by_Period_Thickness.png")

    # ==========================================================
    # TAB 3: CAPABILITY (SPC)
    # ==========================================================
    with tab3:
        st.header("3. Distribution & Process Capability (SPC)")
        st.info("Capability indices (Cp, Cpk) apply from Q4 2025 onwards. Limit calculations based on grades A-B+, A-B only.")
        ...
        if cap_summary_rows:
            ...
            fmt = {'Thickness': '{:.2f}', 'Mean': '{:.2f}', 'Std': '{:.3f}',
            ...
            st.dataframe(cap_df.style.map(color_cpk_cell, subset=['Cpk']).format(fmt, na_rep='—'),
                         use_container_width=True, hide_index=True)
            st.markdown("---")
        ...
        for period in ordered_periods:
            ...
            for idx, f in enumerate([x for x in ['YS', 'TS', 'EL', 'YPE'] if x in df_p.columns]):
                with cols[idx % 2]:
                    ...
                    render_capability_badge(calc_capability(df_p_valid[f].dropna().values, f, period, 'Overall'), f, period, 'Overall')
                    fig, ax = plt.subplots(figsize=(8, 4.5))
                    plot_dist(ax, df_p, f, f"{f} (Overall - {period})", ov_y, period, 'Overall')
                    fig.tight_layout()
                    st.pyplot(fig)
                    download_chart_btn(fig, f"SPC_{period}_Overall_{f}.png")
                    plt.close(fig)
            ...
            for thick in thickness_list:
                ...
                for idx, f in enumerate([x for x in ['YS', 'TS', 'EL', 'YPE'] if x in df_t.columns]):
                    with tcols[idx % 2]:
                        ...
                        render_capability_badge(calc_capability(df_t_valid[f].dropna().values, f, period, thick), f, period, thick)
                        fig, ax = plt.subplots(figsize=(8, 4.5))
                        plot_dist(ax, df_t, f, f"{f} (Thick:{thick} - {period})", ly, period, thick)
                        fig.tight_layout()
                        st.pyplot(fig)
                        download_chart_btn(fig, f"SPC_{period}_{thick}mm_{f}.png")
                        plt.close(fig)
            st.markdown("---")
    ...
    with tab4:
        ...
        st.info("Tracking process stability for 2026 onwards. Grades A-B+, A-B only.")
        ...
        if df_t4.empty:
            ...
            for t4_feat in ['YS', 'TS', 'EL', 'YPE']:
                ...
                lsl, usl, tgt = spec.get('min'), spec.get('max'), spec.get('target')
                if lsl is not None: ax_i.axhline(lsl, color='darkred', linestyle='-', linewidth=2, label=f'LSL: {lsl}')
                if usl is not None: ax_i.axhline(usl, color='darkred', linestyle='-', linewidth=2, label=f'USL: {usl}')
                if tgt is not None: ax_i.axhline(tgt, color='blue', linestyle=':', linewidth=1.5, label=f'Target: {tgt}')
                out_condition = (vals > ucl_i) | (vals < lcl_i)
                if usl is not None: out_condition = out_condition | (vals > usl)
                if lsl is not None: out_condition = out_condition | (vals < lsl)
                out_i = np.where(out_condition)[0]
                if len(out_i) > 0:
                    ax_i.scatter(out_i, vals[out_i], color='red', zorder=5, s=50, label="Out of Bounds")
                all_y_vals = [v for v in [np.max(vals), np.min(vals), ucl_i, lcl_i, usl, lsl] if v is not None]
                y_max = max(all_y_vals); y_min = min(all_y_vals)
                ...
                st.pyplot(fig_imr)
                download_chart_btn(fig_imr, f"IMR_{t4_feat}_{t4_thick}mm.png")
                plt.close(fig_imr)
    ...
    with tab5:
        ...
        if LEN_COL in df.columns and SCRAP_COL in df.columns:
            ...
            trend_data = df_scrap_master.groupby('Time_Group').agg(Input_Length=(LEN_COL, 'sum'), Total_Scrap=(SCRAP_COL, 'sum')).reset_index()
            ...
            if not trend_data.empty:
                ...
                for i, val in enumerate(trend_data['Rejection_Rate (%)']):
                    ax_trend.annotate(f'{val:.2f}%', xy=(i, val), xytext=(0, 8), textcoords="offset points",
            ...
            st.pyplot(fig_trend)
            download_chart_btn(fig_trend, "Rejection_Rate_Trend.png")
            plt.close(fig_trend)

            st.markdown("---")
            st.subheader("Scrap Rate by Time Period")
            scrap_by_period = df_scrap_master.groupby('Time_Group').agg(
                Total_Length=(LEN_COL, 'sum'), Total_Scrap=(SCRAP_COL, 'sum'), Coil_Count=(COIL_ID_COL, 'count')).reset_index()
            scrap_by_period['Scrap_Rate (%)'] = np.where(scrap_by_period['Total_Length'] > 0,
                (scrap_by_period['Total_Scrap'] / scrap_by_period['Total_Length'] * 100), 0).round(2)
            ...
            if not scrap_by_period.empty:
                ...
                for i, val in enumerate(scrap_by_period['Scrap_Rate (%)']):
                    ax_p.annotate(f"{val:.2f}%", xy=(i, val), xytext=(0, 5), textcoords="offset points",
                                  ha='center', va='bottom', fontweight='bold')
                add_chart_border(ax_p)
                plt.xticks(rotation=30, ha='right')
                fig_p.tight_layout()
            st.pyplot(fig_p)
            download_chart_btn(fig_p, "Scrap_Rate_by_Period.png")
            plt.close(fig_p)

            st.dataframe(scrap_by_period.style.background_gradient(subset=['Scrap_Rate (%)'], cmap='Reds')
            ...
            with col_t:
                ...
                fig_t.tight_layout()
                st.pyplot(fig_t)
                download_chart_btn(fig_t, "Scrap_by_Thickness.png")
                plt.close(fig_t)
            ...
            with col_m:
                ...
                fig_m.tight_layout()
                st.pyplot(fig_m)
                download_chart_btn(fig_m, "Scrap_by_Material.png")
                plt.close(fig_m)
            ...
            st.dataframe(scrap_detail.style.background_gradient(subset=['Scrap_Rate (%)'], cmap='Oranges')
    ...
    # ==========================================================
    # TAB 6: CUSTOMER END-USE ANALYSIS
    # ==========================================================
    with tab6:
        ...
        if USAGE_COL and COIL_ID_COL in df.columns and LEN_COL in df.columns and SCRAP_COL in df.columns:
            ...
            if df_t6.empty:
                ...
                # --- Monthly Scrap & Material Stability ---
                ...
                macro_df = df_t6.groupby('Usage_Month').agg(
                    Total_Length=(LEN_COL, 'sum'), Total_Scrap=(SCRAP_COL, 'sum'),
                    Avg_YS=('YS', 'mean'), Avg_TS=('TS', 'mean'), Avg_EL=('EL', 'mean'), Avg_YPE=('YPE', 'mean')
                ...
                cols6 = row1_cols + row2_cols
                features = [('Avg_YS', 'Theoretical YS', '#1f77b4'), ('Avg_TS', 'Theoretical TS', '#2ca02c'),
                            ('Avg_EL', 'Theoretical EL', '#9467bd'), ('Avg_YPE', 'Theoretical YPE', '#ff7f0e')]
                ...
                for idx, (col_name, label, color) in enumerate(features):
                    with cols6[idx]:
                        ...
                        ax1.plot(macro_df['Usage_Month'], macro_df['Scrap_Rate (%)'], color=color1, marker='o', linewidth=2)
                        ...
                        if col_name in macro_df.columns:
                            ...
                            if not feat_valid.empty:
                                ...
                                ax2.plot(macro_df['Usage_Month'], macro_df[col_name], color=color, marker='s', linestyle='--', linewidth=2, alpha=0.8)
                        ...
                        st.pyplot(fig_exec)
                        download_chart_btn(fig_exec, f"Scrap_vs_{col_name}.png")
                        plt.close(fig_exec)
                ...
                matrix_html_inner += "".join(f"<th>{m}</th>" for m in usage_months)
                matrix_html_inner += "</tr></thead><tbody>"
                for prod in prod_periods:
                    matrix_html_inner += f"<tr><th style='background-color: #f1f3f5; color: #333;'>{prod}</th>"
                    for usage in usage_months:
                        row = matrix_dict.get((prod, usage))
                        if not row:
                            matrix_html_inner += "<td style='background-color: #fafafa; color: #aaa; text-align:center; vertical-align:middle;'>No Data</td>"
                        else:
                            ...
                            matrix_html_inner += f"<td style='background-color: {bg_color};'><div class='cell-title'>Scrap: {scrap_rate:.1f}%</div><ul class='grade-list'>{''.join(grade_html)}</ul></td>"
                    matrix_html_inner += "</tr>"
                matrix_html_inner += "</tbody></table>"

                # Hiển thị bảng + nút tải PNG
                st.markdown(matrix_html_inner, unsafe_allow_html=True)
                download_table_btn(matrix_html_inner, "Quality_Matrix.png", "Production vs Usage Quality Matrix")

                # Excel export cho matrix
                import xlsxwriter
                def export_matrix_excel_inline():
                    out = io.BytesIO()
                    wb = xlsxwriter.Workbook(out, {'in_memory': True})
                    ws = wb.add_worksheet("Quality Matrix")
                    hdr = wb.add_format({'bold': True, 'bg_color': '#1a3a5c', 'font_color': 'white', 'border': 1, 'align': 'center', 'valign': 'vcenter', 'font_name': 'Arial', 'font_size': 11})
                    row_hdr = wb.add_format({'bold': True, 'bg_color': '#f1f3f5', 'font_color': '#333', 'border': 1, 'align': 'center', 'font_name': 'Arial'})
                    no_data_fmt = wb.add_format({'font_color': '#aaa', 'bg_color': '#fafafa', 'border': 1, 'align': 'center', 'italic': True, 'font_name': 'Arial'})
                    color_map_xl = {True: '#e8f5e9', False: '#fff3e0'}
                    ws.write(0, 0, "Production \\ Usage", hdr)
                    for ci, m in enumerate(usage_months): ws.write(0, ci + 1, m, hdr)
                    for ri, prod in enumerate(prod_periods):
                        ws.set_row(ri + 1, 70)
                        ws.write(ri + 1, 0, prod, row_hdr)
                        for ci, usage in enumerate(usage_months):
                            row = matrix_dict.get((prod, usage))
                            if not row:
                                ws.write(ri + 1, ci + 1, "No Data", no_data_fmt)
                            else:
                                scrap_rate = row.get('Scrap_Rate', 0)
                                bg = get_color(scrap_rate)
                                cell_fmt = wb.add_format({'bg_color': bg, 'border': 1, 'text_wrap': True, 'valign': 'top', 'font_name': 'Arial', 'font_size': 10})
                                total_coils = row.get('Total_Coils', 0)
                                lines = [f"Scrap: {scrap_rate:.1f}%"]
                                if total_coils and total_coils > 0:
                                    for g in available_grades:
                                        g_pct = (row.get(g, 0) or 0) / total_coils * 100
                                        if g_pct > 0: lines.append(f"{g}: {g_pct:.0f}%")
                                ws.write(ri + 1, ci + 1, "\n".join(lines), cell_fmt)
                    ws.set_column(0, 0, 20)
                    ws.set_column(1, len(usage_months), 18)
                    wb.close()
                    return out.getvalue()

                col_dl1, col_dl2, _ = st.columns([1, 1, 3])
                with col_dl2:
                    xl_bytes = export_matrix_excel_inline()
                    st.download_button("⬇️ Tải Matrix Excel", xl_bytes, "Quality_Matrix.xlsx",
                ...
                with col_h1:
                    ...
                    sns.heatmap(pivot_scrap, annot=True, fmt=".1f", cmap="Reds", linewidths=1,
                                linecolor='white', ax=ax_h1, annot_kws={"size": 10, "weight": "bold"})
                    ax_h1.set_ylabel("Usage Month", fontweight='bold')
                    ax_h1.set_xlabel("Production Period", fontweight='bold')
                    plt.xticks(rotation=45, ha='right')
                    fig_h1.tight_layout()
                    st.pyplot(fig_h1)
                    download_chart_btn(fig_h1, "Scrap_Heatmap.png")
                    plt.close(fig_h1)
                ...
                with col_h2:
                    ...
                    color_map_g = {'A-B+': '#2e7d32', 'A-B': '#1f77b4', 'A-B-': '#ffa726', 'B+': '#ef5350', 'B': '#c62828'}
                    plot_colors = [color_map_g.get(g, '#888') for g in available_grades]
                    ...
                    fig_g2.tight_layout()
                    st.pyplot(fig_g2)
                    download_chart_btn(fig_g2, "Grade_Distribution_Usage.png")
                    plt.close(fig_g2)
            ...
            st.info("Identifying identical coils processed on both Old and New machines.")
            ...
            if not split_pivot.empty and old_machine_col in split_pivot.columns and new_machine_col in split_pivot.columns:
                ...
                conds = [(split_pivot[old_machine_col] > 10) & (split_pivot[new_machine_col] < 5),
                ...
                choices = ["🚨 Old Machine Issue (Proven)", "⚠️ Material / Process Issue",
                           "⚙️ New Machine Tuning Issue", "✅ Improved on New Machine"]
                ...
                fmt_dict = {'Scrap (Old Machine)': '{:.2f}%', 'Scrap (New Machine)': '{:.2f}%', 'Delta (%)': '{:.2f}%',
                            'Theoretical YS': '{:.1f}', 'Theoretical TS': '{:.1f}', 'Theoretical EL': '{:.1f}', 'Theoretical YPE': '{:.1f}'}
                st.dataframe(split_report.style.format(fmt_dict, na_rep="N/A")
