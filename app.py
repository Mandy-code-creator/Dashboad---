...
if uploaded_file is not None:
    ...
    with tab3:
        ...
        if cap_summary_rows:
            ...
            st.dataframe(
                cap_df.style.map(color_cpk_cell, subset=['Cpk']).format(fmt, na_rep='—'),
                use_container_width=True, hide_index=True
            )
            st.markdown("---")
    ...
    with tab4:
        ...
        if df_t4.empty:
            ...
            t4_thick = st.selectbox("Select Thickness Category:", thicknesses, key="t4_thickness")
    
            if t4_thick != 'Overall':
                plot_df_base = df_t4[df_t4['Actual_Thickness'] == t4_thick].copy()
            else:
            ...
            for t4_feat in t4_features:
                ...
                if t4_feat == 'Coating_Thickness_Avg':
                    ...
                    for coating_group in coating_groups:
                        group_df = plot_df_base[plot_df_base['Coating_Group'] == coating_group].copy()
                ...
                for coating_group_name, feature_base_df in group_datasets:
                    plot_df = feature_base_df.sort_values('Production_Date').dropna(subset=[t4_feat]).copy()
                    if len(plot_df) < 2:
                        continue
    
                    st.markdown("---")
                    feature_name = ("Coating Thickness Avg" if t4_feat == 'Coating_Thickness_Avg' else t4_feat)
    
                    if t4_feat == 'Coating_Thickness_Avg':
                        st.subheader(f"Feature: {feature_name} | {coating_group_name}")
                    else:
                        st.subheader(f"Feature: {feature_name}")
    
                    vals_all = plot_df[t4_feat].values
                    cap_thickness = (t4_thick if t4_feat != 'Coating_Thickness_Avg' else 'Overall')
    
                    cap_data = calc_capability(vals_all, t4_feat, '2026-01', cap_thickness)
                    render_capability_badge(cap_data, t4_feat, '2026-01', cap_thickness)
                    ...
                    # SPEC LIMITS
                    if t4_feat == 'Coating_Thickness_Avg':
                        if coating_group_name == '25 min (Estimated)': lsl = 25.0
                        elif coating_group_name == '40 min (Estimated)': lsl = 40.0
                        else: lsl = None
                        usl, tgt = None, None
                    else:
                    ...
                    if lsl is not None: ax_i.axhline(lsl, color='darkred', linestyle='-', linewidth=2, label=f'LSL (Spec Min): {lsl}')
                    if usl is not None: ax_i.axhline(usl, color='darkred', linestyle='-', linewidth=2, label=f'USL (Spec Max): {usl}')
                    ...
                    title_suffix = (f"{t4_thick}mm | {coating_group_name}" if t4_feat == 'Coating_Thickness_Avg' else f"{t4_thick}mm")
                    ax_i.set_title(f"Individual (I) Chart - {feature_name} ({title_suffix})", fontweight='bold')
                    ...
                    if len(out_mr) > 0: ax_mr.scatter(out_mr + 1, mr[out_mr], color='red', zorder=5, s=50)
                    ...
                    try:
                        ...
                        safe_group_name = coating_group_name.replace(" ", "_").replace("(", "").replace(")", "")
                        st.download_button(
                            label=f"📥 Download {feature_name} Chart (.pptx)",
                            data=pptx_stream,
                            file_name=f"IMR_Chart_{t4_feat}_{t4_thick}_{safe_group_name}.pptx",
                        ...
                        st.error("Missing dependency python-pptx for PowerPoint export.")
                    plt.close(fig_imr)
    ...
    with tab5:
        ...
        if LEN_COL in df.columns and SCRAP_COL in df.columns:
            ...
            buf_trend.seek(0)
            st.download_button(label="📥 Download Trend Chart (PNG)", data=buf_trend, file_name="rejection_rate_trend.png", mime="image/png", key="dl_trend")
            plt.close(fig_trend)
            ...
            buf_p.seek(0)
            st.download_button(label="📥 Download Scrap Rate Chart (PNG)", data=buf_p, file_name="scrap_rate_by_period.png", mime="image/png", key="dl_period")
            plt.close(fig_p)
            ...
            with col_t:
                ...
                buf_t.seek(0)
                st.download_button(label="📥 Download Thickness Chart", data=buf_t, file_name="scrap_thickness.png", mime="image/png", key="dl_thick")
                plt.close(fig_t)
            ...
            with col_m:
                ...
                buf_m.seek(0)
                st.download_button(label="📥 Download Material Chart", data=buf_m, file_name="scrap_material.png", mime="image/png", key="dl_mat")
                plt.close(fig_m)
            ...
            st.warning("Required columns ('實測長度' or '尾料剔退') not found.")
    ...
    with tab6:
        ...
        if USAGE_COL and COIL_ID_COL in df.columns and LEN_COL in df.columns and SCRAP_COL in df.columns: 
            ...
            if df_coil.empty:
                ...
                # Monthly Scrap Chart Verification Analysis
                ...
                for idx, (col_name, label, color) in enumerate(features):
                    ...
                    with cols[idx]:
                        ...
                        buf.seek(0)
                        st.download_button(label=f"📸 Download {label} Chart", data=buf, file_name=f"Scrap_vs_{col_name}.png", mime="image/png", type="primary", use_container_width=True, key=f"dl_chart_{idx}")
                        plt.close(fig_exec) 
                ...
                prod_summary = df_coil.groupby('Time_Group').agg({LEN_COL: 'sum', WT_COL: 'sum' if WT_COL in df_coil.columns else lambda x: 0}).to_dict('index')
                usage_summary = df_coil.groupby('Usage_Month').agg({LEN_COL: 'sum', WT_COL: 'sum' if WT_COL in df_coil.columns else lambda x: 0}).to_dict('index')
                ...
                for prod in prod_periods:
                    ...
                    html_parts.append(f"<td class='summary-cell'>L: {p_len:,.0f} m<br>W: {p_wt:,.0f} kg</td></tr>")
                ...
                html_parts.append(f"<td class='summary-cell' style='background-color: #bbdefb; color: #b71c1c;'>Total L: {total_matrix_L:,.0f} m<br>Total W: {total_matrix_W:,.0f} kg</td></tr></tbody></table>")
                ...
                def create_word_report():
                    ...
                    for prod in prod_periods:
                        ...
                        for i, usage in enumerate(usage_months):
                            ...
                            if not row:
                                ...
                                scrap_rate = row['Scrap_Rate']
                                total_coils = row.get('Total_Coils', 0)
                                set_cell_background(cell, get_color(scrap_rate))
                                ...
                                if cell_total_grade > 0 and available_grades:
                                    for g in available_grades:
                                        ...
                                        if g_pct > 0:
                                            ...
                                            run_pct.font.color.rgb = RGBColor(0, 128, 0) if "A" in g else RGBColor(220, 20, 60)
                        ...
                        run_out.font.size, run_out.font.bold = Pt(8), True
                    ...
                    run_grand.font.size, run_grand.font.bold = Pt(9), True
    ...
    with tab7:
        ...
        t7_summary = pd.DataFrame({'Input_Length': monthly_input_len, 'Total_Scrap': monthly_total_scrap}).join(monthly_props).reset_index()
        t7_summary['Scrap_Rate (%)'] = np.where(t7_summary['Input_Length'] > 0, (t7_summary['Total_Scrap'] / t7_summary['Input_Length'] * 100), 0).round(2)
        ...
        for idx, (feat_id, label, color) in enumerate(features_to_plot):
            if feat_id in t7_summary.columns:
                with t7_cols[idx]:
                    ...
                    ax1.plot(t7_summary['Prod_Month'], t7_summary['Scrap_Rate (%)'], color='#d62728', marker='o', linewidth=2.5, label='Scrap Rate')
                    ax1.tick_params(axis='y', labelcolor='#d62728')
                    
                    ax2 = ax1.twinx()
                    ax2.set_ylabel(label, color=color, fontweight='bold')
                    ax2.plot(t7_summary['Prod_Month'], t7_summary[feat_id], color=color, marker='s', linestyle='--', alpha=0.8, label=label)
                    ...
                    buf_t7.seek(0)
                    st.download_button(label=f"📸 Download {label} Chart", data=buf_t7, file_name=f"Prod_Scrap_vs_{feat_id}.png", mime="image/png", type="primary", use_container_width=True, key=f"dl_t7_chart_{idx}")
                    plt.close(fig_t7)
